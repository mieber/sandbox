...
//		compare("player_020.jpg",new String[]{"Teophanus", "SleepyAsh", "PandaAttack", "Guanako", "GeorgeNaruto"}, new String[]{"iShares", "Hoghart", "PrideKinG", "Psychosis", "MitaKei"}, "INFERNAL SHRINES");
	...
	private void compare(String file, String[] expectedAllies, String[] expectedEnemies, String map)
		...
		for (int i = 0; i < allies.size(); i++) {
			...
			if (expectedAllies[i] == null) {
				System.out.print(".\t");
			} else if (expectedAllies[i].equalsIgnoreCase(allies.get(i).getText())) {
				System.out.print("+\t");
				good++;
				possible++;
			} else {
				System.out.print("-\t");
				errors.add(expectedAllies[i] + "\t" + allies.get(i));
				possible++;
			}
		...
		for (int i = 0; i < enemies.size(); i++) {
			...
			if (expectedEnemies[i] == null) {
				System.out.print(".\t");
			} else if (expectedEnemies[i].equalsIgnoreCase(enemies.get(i).getText())) {
				System.out.print("+\t");
				good++;
				possible++;
			} else {
				System.out.print("-\t");
				errors.add(expectedEnemies[i] + "\t" + enemies.get(i));
				possible++;
			}
